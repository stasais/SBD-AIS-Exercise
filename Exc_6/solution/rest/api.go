...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// handle any error!
		object, err := s3.GetObject(r.Context(), "orders", order.GetFilename(), minio.GetObjectOptions{})
		if err != nil {
...
func PostOrder(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = s3.PutObject(r.Context(), "orders", dbOrder.GetFilename(), reader, int64(len(markdownContent)), minio.PutObjectOptions{ContentType: "text/markdown"})
		if err != nil {
			slog.Error("Unable to store receipt to S3", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to store receipt to S3")
			return
		}
		render.Status(r, http.StatusOK)
		render.JSON(w, r, "ok")
